...
public final class ObjectIdentifier implements Serializable {
    ...
    public int[] toIntArray() {
        ...
        while (i2 < length) {
            if ((this.encoding[i2] & 128) == 0) {
                if ((i2 - fromPos) + 1 > 4) {
                    BigInteger big = new BigInteger(pack(this.encoding, fromPos, (i2 - fromPos) + 1, 7, 8));
                    if (fromPos == 0) {
                        i = which + 1;
                        result[which] = 2;
            ...
            if (i >= result.length) {
                result = Arrays.copyOf(result, i + 10);
            }
            i2++;
            which = i;
        }
    ...
    private static byte[] pack(byte[] in, int ioffset, int ilength, int iw, int ow) {
        if (!$assertionsDisabled && (iw <= 0 || iw > 8)) {
            throw new AssertionError("input NUB must be between 1 and 8");
    ...
    private static void check(byte[] encoding) throws IOException {
        ...
        int i = 0;
        while (i < length) {
            if (encoding[i] == DerValue.TAG_CONTEXT && (i == 0 || (encoding[i - 1] & 128) == 0)) {
